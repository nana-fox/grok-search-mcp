...
| 工具 | 参数 | 说明 |
|------|------|------|
| `grok_search` | `query`(必填)<br>`recency?`: `day` \| `week` \| `month`<br>`max_sources?`: 正整数 | 实时搜索网页和 X,返回综合答案 + 来源。Grok 自动决定数据源。请求默认 30s 超时。 |
